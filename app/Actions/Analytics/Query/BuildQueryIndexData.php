...
class BuildQueryIndexData
{
    ...
    public function handle(
        ...
        $queries = $this->fetchQueries($orgId, $projId, $envId, $start, $end, $sort, $direction, $search, $page);
     ...
     * @return array<string, mixed>
     */
    private function fetchQueries(int $orgId, int $projId, int $envId, string $start, string $end, string $sort, string $direction, string $search, int $page): array
    {
        $baseWhere = "WHERE organization_id = {$orgId}
            AND project_id = {$projId}
            AND environment_id = {$envId}
