...
{
    /**
     * Build mail analytics grouped by class + mailer.
     *
     * @return array<string, mixed>
     */
    public function handle(AnalyticsContext $ctx, PeriodResult $period): array
    {
        $rows = DB::table('extraction_mails')
            ...
            ->select([
                ...
                DB::raw('CAST(ROUND(AVG(CASE WHEN failed = 0 THEN duration END) , 2) AS DOUBLE) as avg_duration'),
            ])
            ->groupBy('class', 'mailer')
            ->orderBy('total', 'desc')
            ->paginate(50);
