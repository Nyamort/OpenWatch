...
class BuildMailTypeData
{
    ...
    /**
     * Build graph, stats and paginated runs for a given mail class.
     *
     * @return array<string, mixed>
     */
    public function handle(AnalyticsContext $ctx, PeriodResult $period, string $mailClass, string $sort = 'date', string $direction = 'desc', int $page = 1): array
    {
        $envId = $ctx->environment->id;
        $start = ClickHouseService::escape($period->start);
        $end = ClickHouseService::escape($period->end);
        $escapedClass = ClickHouseService::escape($mailClass);
        ...
        $rows = $this->clickhouse->select("
            SELECT id, recorded_at, execution_source, execution_preview, mailer, subject, `to`, cc, bcc, attachments, duration
            FROM extraction_mails
            {$baseWhere}
