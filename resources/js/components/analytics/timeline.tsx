...
export function executionsToTimelineSpans(
...
const STICKY =
    'sticky top-16 z-10 transition-[top] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:top-12';
const CURSOR_STICKY =
    'pointer-events-none sticky top-25 z-30 h-0 transition-[top] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:top-21';
...
function flattenSpans(
    ...
    for (const span of spans) {
        ...
        if (hasChildren && expandedIds.has(span.id)) {
            result.push(
                ...flattenSpans(span.children!, expandedIds, depth + 1),
            );
        }
    }
    return result;
...
function TimelineLabelRow({
    ...
    return (
        ...
        >
            ...
            <span className="shrink-0 text-[11px] tracking-wider text-white uppercase">
                {span.label}
            </span>
            {span.sublabel && (
    ...
    return (
        <div className={cn('relative shrink-0 overflow-hidden', ROW_HEIGHT)}>
            {span.durationUs === null ? (
                <span
                    className="absolute top-1/2 -translate-y-1/2 pl-1 text-[10px] font-bold tracking-wider text-zinc-500 uppercase"
                    style={{ left: pct(span.offsetUs) }}
                >
                    {span.label}
                    {span.sublabel && (
                        <span className="ml-1.5 font-normal text-zinc-600 normal-case">
                            {span.sublabel}
                        </span>
                    )}
                </span>
                ...
                >
                    <span className="shrink-0 px-2 text-[10px] tracking-wider uppercase">
                        {span.label}
                    </span>
...
export function Timeline({ totalDurationUs, spans, className }: TimelineProps) {
    ...
    const flatSpans = useMemo(
        () => flattenSpans(spans, expandedIds),
        [spans, expandedIds],
    );

    const pct = useCallback(
        (us: number) => `${(us / axisDurationUs) * 100}%`,
        [axisDurationUs],
    );
