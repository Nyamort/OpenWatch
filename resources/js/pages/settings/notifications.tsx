...
const CATEGORY_DESCRIPTIONS: Record<string, string> = {
    ...
    security: 'Important security notifications about your account. Cannot be disabled.',
};
...
export default function Notifications({ categories, status }: NotificationsProps) {
                ...
                <div className="space-y-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-4">
                            {Object.entries(categories).map(([category, pref]) => (
                                <div
                                    key={category}
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        role="switch"
                                        aria-checked={data.categories[category]?.enabled ?? pref.enabled}
                                        disabled={pref.locked}
                                        onClick={() =>
                                            handleToggle(
                                                category,
                                                !(data.categories[category]?.enabled ?? pref.enabled),
                                            )
                                        }
                                        className={[
                                            ...
                                            (data.categories[category]?.enabled ?? pref.enabled)
                                                ? 'bg-primary'
                                                : 'bg-input',
                                            pref.locked ? 'cursor-not-allowed opacity-50' : '',
                                        ]
                                            .filter(Boolean)
                                            .join(' ')}
                                    >
