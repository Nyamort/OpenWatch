...
export function QueryCharts({ graph, stats }: QueryChartsProps) {
        ...
        <div className="flex gap-4 text-sm">
            <div className="flex flex-col items-end gap-0.5">
                <span className="text-muted-foreground flex items-center gap-1">
                    <span className="inline-block h-3 w-1 rounded-sm" style={{ backgroundColor: callsChartConfig.calls.color }} />
                    Calls
                </span>
    ...
    return (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            >
                {(legendContent) => (
                    <BarChart syncId="queries" data={graph} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        ...
                        <ChartTooltip
                            ...
                            content={({ active, label, payload }) => (
                                <AnalyticsTooltip
                                    ...
                                    label={label}
                                    rows={[
                                        { color: callsChartConfig.calls.color, label: 'Calls', value: payload?.find(p => p.dataKey === 'calls')?.value ?? 0 },
                                    ]}
                                />
                            )}
                        />
                        <ChartLegend verticalAlign="top" content={legendContent} />
                        <Bar dataKey="calls" fill={callsChartConfig.calls.color} radius={[3, 3, 0, 0]} />
                    </BarChart>
                )}
            </ChartPanel>
            ...
            >
                {(legendContent) => (
                    <AreaChart syncId="queries" data={graph} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        <defs>
                            <linearGradient id="fillQryAvg" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <linearGradient id="fillQryP95" x1="0" y1="0" x2="0" y2="1">
                        ...
                        />
                        <ChartLegend verticalAlign="top" content={legendContent} />
                        <Area type="linear" dataKey="p95" stroke={durationChartConfig.p95.color} strokeWidth={2} fill="url(#fillQryP95)" dot={isolatedDot(graph, 'p95', durationChartConfig.p95.color)} />
                        <Area type="linear" dataKey="avg" stroke={durationChartConfig.avg.color} strokeWidth={2} fill="url(#fillQryAvg)" dot={isolatedDot(graph, 'avg', durationChartConfig.avg.color)} />
                    </AreaChart>
                )}
            </ChartPanel>
        </div>
