...
import type { OutgoingRequestHostRow, OutgoingRequestSortKey, Pagination, SortDir } from '../types';
...
export function OutgoingRequestTable({ hosts, pagination, sort, direction, search }: OutgoingRequestTableProps) {
    const { props } = usePage();
    ...
    const { searchValue, handleSearch, handlePage, handleSort } = useAnalyticsTable<OutgoingRequestSortKey>({
        search,
        only: ['hosts', 'pagination', 'sort', 'direction'],
    });

    const onSort = (col: string) => handleSort(col as OutgoingRequestSortKey, sort, direction);

    function handleRowClick(host: string) {
        router.visit(hostRoute.url(
            { organization: activeOrganization.slug, project: activeProject.slug, environment: activeEnvironment.slug },
            { query: { host } },
        ));
    }
            ...
            <Table className="border-separate border-spacing-y-1.5" containerClassName="overflow-x-visible">
                <TableHeader className="sticky top-16 z-10 backdrop-blur-sm [&_tr]:border-0">
                    <TableRow className="border-0 hover:bg-transparent shadow-sm shadow-black/4 [&_th]:border-y [&_th]:border-border [&_th:first-child]:border-l [&_th:first-child]:rounded-l-lg [&_th:last-child]:border-r [&_th:last-child]:rounded-r-lg [&_th]:bg-muted/50">
                        <SortableHead column="host" sort={sort} direction={direction} onSort={onSort} className="h-11 px-5 text-xs font-medium">
                            Host
                        </SortableHead>
                        <SortableHead column="success" sort={sort} direction={direction} onSort={onSort} align="right" className="h-11 w-px whitespace-nowrap px-4 text-right text-xs font-medium">
                            1/2/3xx
                        </SortableHead>
                        <SortableHead column="count_4xx" sort={sort} direction={direction} onSort={onSort} align="right" className="h-11 w-px whitespace-nowrap px-4 text-right text-xs font-medium">
                            4xx
                        </SortableHead>
                        <SortableHead column="count_5xx" sort={sort} direction={direction} onSort={onSort} align="right" className="h-11 w-px whitespace-nowrap px-4 text-right text-xs font-medium">
                            5xx
                        </SortableHead>
                        ...
                        hosts.map((row) => (
                            ...
                            >
                                <TableCell className="h-11 max-w-xs overflow-hidden px-5">
                                    <span className="block truncate font-mono text-sm">{row.host}</span>
                                </TableCell>
                                <TableCell className="h-11 w-px whitespace-nowrap px-4 text-right tabular-nums">
                                    {row.success.toLocaleString()}
                                </TableCell>
                                <TableCell className={`h-11 w-px whitespace-nowrap px-4 text-right tabular-nums ${row.count_4xx === 0 ? 'text-muted-foreground' : 'text-amber-500'}`}>
                                    <span className="flex items-center justify-end gap-1">
                                        {row.count_4xx > 0 && <TriangleAlert className="size-3" />}
                                        {row.count_4xx.toLocaleString()}
                                    </span>
                                </TableCell>
                                <TableCell className={`h-11 w-px whitespace-nowrap px-4 text-right tabular-nums ${row.count_5xx === 0 ? 'text-muted-foreground' : 'text-red-500'}`}>
                                    <span className="flex items-center justify-end gap-1">
                                        {row.count_5xx > 0 && <OctagonAlert className="size-3 shrink-0" />}
