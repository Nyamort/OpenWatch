...
export function OutgoingRequestCharts({ graph, stats }: OutgoingRequestChartsProps) {
    const requestsStats = (
        <div className="flex gap-4 text-sm">
            {(['success', 'count_4xx', 'count_5xx'] as const).map((key) => (
                <div key={key} className="flex flex-col items-end gap-0.5">
                    <span className="text-muted-foreground flex items-center gap-1">
                        <span className="inline-block h-3 w-1 rounded-sm" style={{ backgroundColor: requestsChartConfig[key].color }} />
    ...
    return (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            >
                {(legendContent) => (
                    <BarChart syncId="outgoing" data={graph} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        ...
                        <ChartTooltip
                            ...
                            content={({ active, label, payload }) => (
                                <AnalyticsTooltip
                                    ...
                                    label={label}
                                    rows={[
                                        { color: requestsChartConfig.success.color, label: '1/2/3xx', value: payload?.find(p => p.dataKey === 'success')?.value ?? 0 },
                                        { color: requestsChartConfig.count_4xx.color, label: '4xx', value: payload?.find(p => p.dataKey === 'count_4xx')?.value ?? 0 },
                                        { color: requestsChartConfig.count_5xx.color, label: '5xx', value: payload?.find(p => p.dataKey === 'count_5xx')?.value ?? 0 },
                                    ]}
                                />
                            )}
                        />
                        <ChartLegend verticalAlign="top" content={legendContent} />
                        <Bar dataKey="success" stackId="a" fill={requestsChartConfig.success.color} />
                        <Bar dataKey="count_4xx" stackId="a" fill={requestsChartConfig.count_4xx.color} />
                        <Bar dataKey="count_5xx" stackId="a" fill={requestsChartConfig.count_5xx.color} radius={[3, 3, 0, 0]} />
                    </BarChart>
                )}
            </ChartPanel>
            ...
            >
                {(legendContent) => (
                    <AreaChart syncId="outgoing" data={graph} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        <defs>
                            <linearGradient id="fillOutgoingAvg" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <linearGradient id="fillOutgoingP95" x1="0" y1="0" x2="0" y2="1">
                        ...
                        />
                        <ChartLegend verticalAlign="top" content={legendContent} />
                        <Area type="linear" dataKey="p95" stroke={durationChartConfig.p95.color} strokeWidth={2} fill="url(#fillOutgoingP95)" dot={isolatedDot(graph, 'p95', durationChartConfig.p95.color)} />
                        <Area type="linear" dataKey="avg" stroke={durationChartConfig.avg.color} strokeWidth={2} fill="url(#fillOutgoingAvg)" dot={isolatedDot(graph, 'avg', durationChartConfig.avg.color)} />
                    </AreaChart>
                )}
            </ChartPanel>
        </div>
