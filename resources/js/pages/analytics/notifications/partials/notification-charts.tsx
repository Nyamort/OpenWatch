...
export function NotificationCharts({ graph, stats }: NotificationChartsProps) {
    ...
    return (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            >
                {(legendContent) => (
                    <BarChart syncId="notifications" data={graph} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        ...
                        <ChartTooltip
                            ...
                            content={({ active, label, payload }) => (
                                <AnalyticsTooltip
                                    ...
                                    label={label}
                                    rows={[
                                        { color: notificationsChartConfig.count.color, label: 'Notifications', value: payload?.find(p => p.dataKey === 'count')?.value ?? 0 },
                                    ]}
                                />
                            )}
                        />
                        <ChartLegend verticalAlign="top" content={legendContent} />
                        <Bar dataKey="count" fill={notificationsChartConfig.count.color} radius={[3, 3, 0, 0]} />
                    </BarChart>
                )}
            </ChartPanel>
            ...
            >
                {(legendContent) => (
                    <AreaChart syncId="notifications" data={graph} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                        <defs>
                            <linearGradient id="fillNotifAvg" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <linearGradient id="fillNotifP95" x1="0" y1="0" x2="0" y2="1">
                        ...
                        />
                        <ChartLegend verticalAlign="top" content={legendContent} />
                        <Area type="linear" dataKey="p95" stroke={durationChartConfig.p95.color} strokeWidth={2} fill="url(#fillNotifP95)" dot={isolatedDot(graph, 'p95', durationChartConfig.p95.color)} />
                        <Area type="linear" dataKey="avg" stroke={durationChartConfig.avg.color} strokeWidth={2} fill="url(#fillNotifAvg)" dot={isolatedDot(graph, 'avg', durationChartConfig.avg.color)} />
                    </AreaChart>
                )}
            </ChartPanel>
        </div>
